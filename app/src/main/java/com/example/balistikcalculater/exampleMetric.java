...
public class exampleMetric {
	...
	public static void main(String[] args){
        ...
        double zero=convertMesure.mToYards(100); // The zero range of the rifle, in yards.
        ...
        for (s=0;s<=100;s++){
            ...
            int valeurRecherche = (int)Math.abs(convertMesure.mToYards(s* incrementation));

            xValue = (int)Math.abs(retrieveValue.getRange(sln, valeurRecherche));
            yValue = retrieveValue.getPath(sln,valeurRecherche);
            System.out.println("X: " + s* incrementation + " || Y: " + df.format(convertMesure.inchToCm(yValue)) + " || MOA: " + df.format(retrieveValue.getMOA(sln,valeurRecherche)) + " || Vitesse: " + df.format(convertMesure.fsToMs(retrieveValue.getVelocity(sln,valeurRecherche))));
        }
	}
}
