...
public class Zero {
    private double GRAVITY = (-32.18);
    private AngleConversion conversionAngle = new AngleConversion();
    private Retard valuRetard = new Retard();
    ...
    double ZeroAngle(int DragFunction, double DragCoefficient, double Vi, double SightHeight, double ZeroRange, double yIntercept){
        ...
        da=conversionAngle.degToRad(14);
        ...
        for (angle=0;quit==0;angle=angle+da){
            ...
            for (t=0,x=0,y=-SightHeight/12;x<=ZeroRange*3;t=t+dt){
                ...
                dv = valuRetard.retard(DragFunction, DragCoefficient, v);
                dvy = -dv*vy/v*dt;
                dvx = -dv*vx/v*dt;
            ...
            if (Math.abs(da) < conversionAngle.moaToRad(0.01)) quit=1; // If our accuracy is sufficient, we can stop approximating.
            if (angle > conversionAngle.degToRad(45)) quit=1; // If we exceed the 45 degree launch angle, then the projectile just won't get there, so we stop trying.
    
        }
    
    
        return conversionAngle.radToDeg(angle); // Convert to degrees for return value.
    } 
}